...
public:
    ...
    const thorin::Def* rev_diff(const thorin::Def* def) {
        auto test = ad_imp::op_autodiff(def);
        return test;
    }
...
const Def* PrefixExpr::remit(CodeGen& cg) const {
    switch (tag()) {
        ...
        case DEC: {
            ...
            if (is_int(type()))
                val = core::op(tag() == INC ? core::wrap ::add : core::wrap ::sub, type2wmode(type()), val, one,
                               cg.loc2dbg(loc()));
            else
                val = math::op(tag() == INC ? math::arith::add : math::arith::sub, math::Mode::none, val, one,
                               cg.loc2dbg(loc()));
        ...
        case NOT: return core::op_negate(rhs()->remit(cg), cg.loc2dbg(loc()));
        case TILDE: {
...
const Def* PostfixExpr::remit(CodeGen& cg) const {
    ...
    if (is_int(type()))
        val = core::op(tag() == INC ? core::wrap ::add : core::wrap ::sub, type2wmode(type()), res, one,
                       cg.loc2dbg(loc()));
    else
        val =
            math::op(tag() == INC ? math::arith::add : math::arith::sub, math::Mode::none, res, one, cg.loc2dbg(loc()));
    cg.store(var, val, loc());
    return res;
}
        ...
        if (auto path = callee->isa<PathExpr>()) {
            if (auto fn_decl = path->value_decl()->isa<FnDecl>()) {
                ...
                if (name == "exp") {
                    ...
                    return math::op(math::extrema::fmax, math::Mode::fast, arg(0)->remit(cg), arg(1)->remit(cg),
                                    cg.loc2dbg(loc()));
                } else if (name == "lgamma") {
                ...
                if (auto fn_decl = path->value_decl()->isa<FnDecl>()) {
                    if (fn_decl->is_extern() && fn_decl->abi() == "\"thorin\"") {
                        ...
                        if (name == "bitcast") {
                            ...
                            return cg.world.extract(cg.world.tuple({arg(2)->remit(cg), arg(1)->remit(cg)}),
                                                    arg(0)->remit(cg), cg.loc2dbg(loc()));
                        } else if (name == "insert") {
                            return core::insert_unsafe(arg(0)->remit(cg), arg(1)->remit(cg), arg(2)->remit(cg),
                                                       cg.loc2dbg(loc()));
                        } else if (name == "alignof") {
                            return core::op_bitcast(
                                cg.world.type_int(32),
                                core::op(core::trait::align, cg.convert(type_expr->type_arg(0)), cg.loc2dbg(loc())));
                        } else if (name == "sizeof") {
                            return core::op_bitcast(
                                cg.world.type_int(32),
                                core::op(core::trait::size, cg.convert(type_expr->type_arg(0)), cg.loc2dbg(loc())));
                        } else if (name == "undef") {
                            return cg.world.bot(cg.convert(type_expr->type_arg(0)), cg.loc2dbg(loc()));
                        } else if (name == "reserve_shared") {
                            auto ptr  = cg.convert(type());
                            auto cn   = cg.world.cn({mem::type_mem(cg.world), cg.world.type_int(32),
                            ...
                            auto cn   = cg.world.cn({mem::type_mem(cg.world), cg.world.type_int(32), ptr, poly_type,
                            ...
                            auto cn =
                                cg.world.cn({mem::type_mem(cg.world), ptr, poly_type, poly_type,
                            ...
                            auto cn          = cg.world.cn({mem::type_mem(cg.world), string_type, poly_type,
                            ...
                            auto cn        = cg.world.cn({mem::type_mem(cg.world), poly_type,
...
const Def* ForRangeExpr::remit(CodeGen& cg) const {
    ...
    auto loop_body_wrapper =
        cg.world.nom_lam(cg.world.cn({loop_index_ty, mem_ty, cg.world.cn(mem_ty)}), loop_body->dbg());
    ...
    auto affine_for = affine::op_for(cg.world, begin_def, end_def, cg.world.lit_idx(size, 1), {cg.cur_mem},
                                     loop_body_wrapper, break_bb);
...
const Def* RevDiffExpr::remit(CodeGen& cg) const { return cg.rev_diff(expr()->remit(cg)); }
